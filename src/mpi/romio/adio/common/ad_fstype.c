...
#ifdef ROMIO_TRACE
   { &ADIO_TRACE_operations, ADIO_TRACE, "trace:", "trace"}, 
#endif
#ifdef ROMIO_ICACHE
	...
	ADIOI_Fns **ops, int *error_code)
{
    ...
    *error_code = MPI_SUCCESS;
    *fstype = -1;
    char myname[] = "ADIO_FileSysType_prefix";

    /* search table for prefix */
    cpy = ADIOI_Strdup (filename);
    ADIOI_Strlower (cpy);

    i=0;
    while (fstypes[i].fileops)
    {
	if (!strncmp (fstypes[i].prefix, cpy, strlen(fstypes[i].prefix)))
	{
			  ...
			  ADIOI_Fns **ops, int *error_code)
{
	...
	*error_code = MPI_SUCCESS;
    ...
    /* verify that we support this file system: look for filesystem in the
     * fstypes tables */
    if (! (*ops))
    {
	int i=0;
